...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['addToCart'])) {
        ...
        $existSql = "SELECT * FROM `viewcart` WHERE donutId = '$itemId' AND `userId`='$userId'";
        ...
        if($numExistRows > 0){
            ...
            $sql = "INSERT INTO `viewcart` (`donutId`, `itemQuantity`, `userId`, `addedDate`) VALUES ('$itemId', '1', '$userId', current_timestamp())";   
    ...
    if(isset($_POST['removeItem'])) {
        $itemId = $_POST["itemId"];
        $sql = "DELETE FROM `viewcart` WHERE `donutId`='$itemId' AND `userId`='$userId'";   
        $result = mysqli_query($conn, $sql);
        ...
        if (password_verify($password, $passRow['password'])){ 
            ...
            if ($result){
                ...
                while($addrow = mysqli_fetch_assoc($addResult)){
                    $donutId = $addrow['donutId'];
                    $itemQuantity = $addrow['itemQuantity'];
                    $itemSql = "INSERT INTO `orderitems` (`orderId`, `donutId`, `itemQuantity`) VALUES ('$orderId', '$donutId', '$itemQuantity')";
                ...
                echo '<script>alert("Thanks for ordering with us. Your order id is ' .$orderId. '.");
                    window.location.href="http://localhost/donut_shop/index.php";  
                    </script>';
                    exit();
            }
        } 
    ...
    if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
    {
        ...
        $qty = $_POST['quantity'];
        $updatesql = "UPDATE `viewcart` SET `itemQuantity`='$qty' WHERE `donutId`='$donutId' AND `userId`='$userId'";
