...
<body>
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="card" style="width: 18rem;">
                        <img src="img/card-'.$catId. '.jpg" class="card-img-top" alt="image for this donut" width="249px" height="270px">
                        <div class="card-body">
                            <h5 class="card-title"><a href="index.php?catid=' . $catId . '">' . $catname . '</a></h5>
                            <p class="card-text">' . substr($catdesc, 0, 29). '...</p>
                            <a href="index.php?catid=' . $catId . '" class="btn btn-primary">View All</a>
                        </div>
                    </div>
                </div>';
            }
    ...
    <div class="container my-3" id="cont">
        ...
        <?php 
            $query = $_GET["search"];
            $sql = "SELECT * FROM `donut` WHERE MATCH(donutName, donutDesc) against('$query')"; 
            $result = mysqli_query($conn, $sql);
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="card" style="width: 18rem;">
                        <img src="img/donut-'.$donutId. '.jpg" class="card-img-top" alt="image for this donut" width="249px" height="270px">
                        <div class="card-body">
                            <h5 class="card-title">' . substr($donutName, 0, 20). '...</h5>
                            <h6 style="color: #ff0000">Rs. '.$donutPrice. '/-</h6>
                            <p class="card-text">' . substr($donutDesc, 0, 29). '...</p>
                            <div class="row justify-content-center">';
                                if($loggedin){
                                    ...
                                    if($quaExistRows == 0) {
                                        echo '<form action="partials/_manageCart.php" method="POST">
                                              <input type="hidden" name="itemId" value="'.$donutId. '">
                                ...
                                echo '</form>
                                <a href="viewdonut.php?donutid=' . $donutId . '"><button class="btn btn-primary">Quick View</button></a>
                            </div>
                        </div>
                    </div>
                </div>';
